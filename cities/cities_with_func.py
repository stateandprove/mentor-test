...
# функция получает случайный город из списка, добавляет к выбранным и удаляет его из общей выборки
def pick_random_city(cities: list) -> str:
    random_index = randint(0, len(cities) - 1)
    random_city = cities[random_index]
    cities.pop(random_index)
    picked_cities.append(random_city)
    return random_city
...
# выбираем случайный город из этого списка и заводим переменную current_city для цикла
current_city = pick_random_city(random_first_letter_cities)
...
while failures_count > 0:
    ...
    picked_cities.append(city_input_lower)
    ...
    # берем новый случайный город из списка
    current_city = pick_random_city(new_cities_list)
